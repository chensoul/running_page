...
const titleForRun = (run: Activity): string => {
  ...
  if (runHour >= 0 && runHour <= 10) {
    return city + ',' + RUN_TITLES.MORNING_TITLE;
  }
  if (runHour > 10 && runHour <= 14) {
    return city + ',' + RUN_TITLES.MIDDAY_TITLE;
  }
  if (runHour > 14 && runHour <= 18) {
    return city + ',' + RUN_TITLES.AFTERNOON_TITLE;
  }
  if (runHour > 18 && runHour <= 21) {
    return city + ',' + RUN_TITLES.EVENING_TITLE;
  }
  return city + ',' + RUN_TITLES.NIGHT_TITLE;
};
