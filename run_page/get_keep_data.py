#!/usr/bin/env python3
"""
从 Keep 读取跑步数据并生成与 Garmin 脚本兼容的 running.json，保存到 assets 目录。
...
def _build_segments_from_keep_api(
    ...
    try:
        ...
        from keep_sync import decode_runmap_data, find_nearest_hr
    except Exception as e:
        logger.debug("无法加载 keep_sync/haversine，跳过 segments: %s", e)
        return []

    try:
        points = decode_runmap_data(geo_points_raw, True)
    except Exception as e:
    ...
    for p in points:
        ...
        p["_ts"] = int(ts)
        if decoded_hr_data:
            hr = find_nearest_hr(decoded_hr_data, p["_ts"], start_time_ms)
            p["_hr"] = hr
        else:
            p["_hr"] = None
...
def fetch_keep_runs_via_api(
    ...
    run_page_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    session, headers = login(session, mobile, password)
    ...
    for i, run_id in enumerate(run_ids):
        try:
            raw = get_single_run_data(session, headers, run_id, sport_type)
...
def parse_args():
    ...
    p.add_argument(
        "--output",
        default="assets/running.json",
        help="输出 JSON 路径 (默认: assets/running.json)",
    )
    p.add_argument(
        "--mobile",
